...
while True:
    ...
    if len(lmList) != 0:
        ...
        # distance between middle and palm bottom
        l1 = math.hypot(x4 - x3, y4 - y3)

        if int(l1) < 130:
            p.press('right')
        # distance between ring and palm bottom
        l2 = math.hypot(x5 - x3, y5 - y3)
        if int(l2) < 110:
            p.press('left')
        # distance between small and palm bottom
        l3 = math.hypot(x6 - x3, y6 - y3)
        if int(l3) < 110:
            p.press('space')

        l4 = math.hypot(x1 - x3, y1 - y3)
        print(l4)
        if int(l4) < 100:
            p.hotkey('alt', 'f4')
